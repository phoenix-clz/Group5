...
const RetirementPlanner = () => {
  ...
  const calculateProjection = () => {
    ...
    for (let month = 0; month <= monthsUntilRetirement; month++) {
      ...
      const inflationAdjustedBalance =
        balance / Math.pow(1 + monthlyInflationRate, month);
    ...
    const inflationAdjustedFinalBalance =
      projectionData[projectionData.length - 1].inflationAdjustedBalance;
    const monthlyIncomeFromSavings =
      (finalBalance * (expectedReturn / 100 / 12)) /
      (1 - Math.pow(1 + expectedReturn / 100 / 12, -yearsUntilRetirement * 12));
    const inflationAdjustedMonthlyIncome =
      monthlyIncomeFromSavings /
      Math.pow(1 + monthlyInflationRate, monthsUntilRetirement);
  ...
  const generatePDF = () => {
    ...
    yPos += 7;
    doc.text(`Retirement Age: ${retirementAge}`, 30, yPos);
    yPos += 7;
    doc.text(
      `Monthly Contribution: Rs. ${monthlyContribution.toLocaleString()}`,
      30,
      yPos
    );
    yPos += 7;
    doc.text(
      `Current Savings: Rs. ${currentSavings.toLocaleString()}`,
      30,
      yPos
    );
    yPos += 7;
    doc.text(`Expected Annual Return: ${expectedReturn}%`, 30, yPos);
    yPos += 7;
    doc.text(`Inflation Rate: ${inflationRate}%`, 30, yPos);
    yPos += 7;
    doc.text(
      `Desired Monthly Income: Rs. ${desiredMonthlyIncome.toLocaleString()}`,
      30,
      yPos
    );
    yPos += 15;
    ...
    doc.setFontSize(12);
    doc.text(
      `Estimated Final Balance: Rs. ${projection.finalBalance.toLocaleString(
        undefined,
    ...
    yPos += 7;
    doc.text(
      `Inflation-Adjusted Final Balance: Rs. ${projection.inflationAdjustedFinalBalance.toLocaleString(
        undefined,
        { maximumFractionDigits: 0 }
      )}`,
      30,
    ...
    yPos += 7;
    doc.text(
      `Estimated Monthly Income: Rs. ${projection.monthlyIncomeFromSavings.toLocaleString(
        undefined,
        { maximumFractionDigits: 0 }
      )}`,
      30,
    ...
    yPos += 7;
    doc.text(
      `Inflation-Adjusted Monthly Income: Rs. ${projection.inflationAdjustedMonthlyIncome.toLocaleString(
        undefined,
        { maximumFractionDigits: 0 }
      )}`,
      30,
    ...
    if (projectionData) {
      const tableData = projectionData
        .filter((data, index) => index % 12 === 0)
        .map((data) => [
          Math.round(data.year),
          data.balance.toLocaleString(undefined, { maximumFractionDigits: 0 }),
          data.inflationAdjustedBalance.toLocaleString(undefined, {
            maximumFractionDigits: 0,
          }),
        ]);
  ...
  const chartData = projectionData
    ? {
        labels: projectionData
          .filter((data, index) => index % 12 === 0)
          .map((data) => Math.round(data.year)),
        datasets: [
          {
          ...
          },
          {
            ...
            data: projectionData
              .filter((data, index) => index % 12 === 0)
              .map((data) => data.inflationAdjustedBalance),
                  ...
                  <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p className="text-2xl text-blue-600">
                        ...
                        {projectionData[
                          projectionData.length - 1
                        ].inflationAdjustedBalance.toLocaleString(undefined, {
                          maximumFractionDigits: 0,
                        })}
                      </p>
                    </div>
