...
import { FaSignOutAlt, FaTachometerAlt, FaBars, FaTimes, FaCalculator, FaPiggyBank } from "react-icons/fa"; // Add the icon here
    ...
    <header className="fixed w-full z-20 p-4 px-12 text-white bg-[#16171C]">
      <nav className="flex items-center justify-between">
        ...
        <div className={`hidden items-center md:flex ${menuOpen ? 'block' : 'hidden'} md:block`}>
          <Link to="/calculator" className="flex items-center px-4 text-md">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  {user.photoURL ? (
                    ...
                    <span className="flex items-center justify-center w-10 h-10 mr-2 bg-gray-400 rounded-full">
                      {/* {user.displayName[0]} */}
                    </span>
                  )}
                </div>
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-[#16171C] py-2 md:hidden">
          <Link to="/calculator" className="block px-4 py-2 text-white text-md">
            Calculator
          </Link>
          {user ? (
            <>
              <Link
                to="/dashboard"
                className="block px-4 py-2 text-white text-md"
              >
                Dashboard
              </Link>
